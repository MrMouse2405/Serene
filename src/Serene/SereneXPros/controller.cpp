...
#define INVALID_TYPE_ERROR                                           \
    luaL_error(L, "Serene/Controller: Invalid Action {%s}", string); \
    return 0;
#define END );
...
func get_corresponding_trigger(lua_State *L, const char *string)
{
    ...
    case 'R':
        ...
        {
        case 'X':
            lambda_a E_CONTROLLER_ANALOG_RIGHT_X end case 'Y' : lambda_a E_CONTROLLER_ANALOG_RIGHT_Y end case '1' : lambda_d E_CONTROLLER_DIGITAL_R1 end case '2' : lambda_d E_CONTROLLER_DIGITAL_R2 end case '\0' : lambda_d E_CONTROLLER_DIGITAL_RIGHT end default : INVALID_TYPE_ERROR
        }
    case 'L':
        switch (string[1])
        {
        case 'X':
            lambda_a E_CONTROLLER_ANALOG_LEFT_X end case 'Y' : lambda_a E_CONTROLLER_ANALOG_LEFT_Y end case '1' : lambda_d E_CONTROLLER_DIGITAL_L1 end case '2' : lambda_d E_CONTROLLER_DIGITAL_L2 end case '\0' : lambda_d E_CONTROLLER_DIGITAL_LEFT end default : INVALID_TYPE_ERROR
        }
    case 'A':
        lambda_d E_CONTROLLER_DIGITAL_A end case 'B' : lambda_d E_CONTROLLER_DIGITAL_B end case 'X' : lambda_d E_CONTROLLER_DIGITAL_X end case 'Y' : lambda_d E_CONTROLLER_DIGITAL_Y end case 'U' : lambda_d E_CONTROLLER_DIGITAL_UP end case 'D' : lambda_d E_CONTROLLER_DIGITAL_DOWN end default : INVALID_TYPE_ERROR
    }
}
...
static int bind_action(lua_State *L)
{
    ...
    if (lua_isstring(L, 2))
    {
        ...
        EventLoop::SereneEvent event(
            // our event trigger
            iz_digital ? (std::function<int(int &)>)
                         // for digital
            ...
            // our event call back
            iz_digital ? (std::function<void(int, int)>)
                // for digital
...
static int bind_periodic(lua_State *L)
{
    ...
    if (lua_isstring(L, 2))
    {
        ...
        EventLoop::SerenePeriodicEvent event(
            // our event call back
            is_digital(s) ? (std::function<void(const int)>)
                            // Digital
                            [ L, EventTrigger, self ](const int reg_key)
                                ->void {
                                    // load our function
                                    lua_rawgeti(L, LUA_REGISTRYINDEX, reg_key);
                                    // call it + error handling
...
void create_controller_instance(lua_State *L)
{
    ...
    */
    ...
    new(&new_controller->registery_keys) std::vector<int>();
